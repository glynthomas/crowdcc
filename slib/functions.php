...
/* note : if functions.php in the /slib directory : use */

// include_once("config.inc.php");
// include_once("constants.inc.php");

/* access to crowdcc shed db */  
require_once($_SERVER["DOCUMENT_ROOT"].'/../db/db.app.shed.conn.php');

/* DBi::$shed * DBi::$conn->query(...) */
...
if (mysqli_num_rows( pjs_mysqli_query("SHOW TABLES LIKE '".LOGS_TABLE."'"))==0)
 ...
 $result=pjs_mysqli_query("SHOW COLUMNS FROM ".LOGS_TABLE." LIKE 'date_added' ");

 if (!mysqli_num_rows($result)) $result=pjs_mysqli_query("ALTER TABLE ".LOGS_TABLE." ADD date_added int AFTER id, CHANGE `id` `id` INT( 11 ) NOT NULL AUTO_INCREMENT "); 
}

function pjs_mysqli_query($q) {

 /* check connection */
 if (DBi::$shed->connect_errno AND DEBUG) {
     echo "<hr>MySQLi ERROR: ".DBi::$shed->connect_error."<hr>";
     exit();
 }

 /* Select queries return a resultset * close * after return */
 if ($result = DBi::$shed->query($q)) {
...
function time_unit($time_interval)
{
 ...
 if ( ($time_interval > (23 * 3600)) AND ($time_interval <= (6 * 86400)) )
 {
  ...
  $unit[1]="<font color=\"#CC0000\">day(s)</font>";
 }
...
function db_close() {
 ...
 if ( DBi::$shed ) $result = mysqli_close( DBi::$shed );
}
...
function show_jobs()
{
 ...
 if (mysqli_num_rows($result))  // check has got some
 {
  ...
  while ($i < mysqli_num_rows($result))
  {
   ...
   $run_only_once_txt= (mysqli_result($result,$i, 'run_only_once'))? "<i><font color=\"#ff0000\"> Will run just once</font></i>":"";
   $time_interval = time_unit($time_interval);
   if ($time_last_fired==0)
   {
    $last_fire_hours = "<font color=\"#CC0000\">NOT yet fired</font>";
    $last_fire_date = "";
   }
   else
   {
...
function show_logs($qstart)
{
 ...
 if (mysqli_num_rows($result))  // check has got some
  ...
  {
   $id=mysqli_result($result,$i, 'id');
   $date_added=mysqli_result($result,$i, 'date_added');
  ...
  echo '<p align="center"><font color="#CC0000">* Maximum length of output will be
...
function fire_remote_script($url)
{
  ...
  $useragent="phpSched (http://open.fu/sched/)";
 ...
 echo '<script src="'.$version_url.'" type="text/javascript"></script>
       <script language="JavaScript"><!--
        ...
        else
        {
          ...
          version_txt=version_txt+"<br>Please <a href=http://open.fu/>visit here</a> ";
...
function Clear($id)
{
 ...
 $result=pjs_mysqli_query("UPDATE ".PJS_TABLE." SET currently_running = '0' where id='$id' ");
 /* db_close(); * moved to class scriptStatus * AFTER results returned * line 373 */
}
