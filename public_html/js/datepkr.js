...
| formatDate(milliseconds, dateFormat) * addEvent(element, eventType, callback), removeEvent(element, eventType, callback) * buildNode(nodeName, attributes, content) * buildCalendar().
| crowdcc.com/use */

var datepkr = (function() {
	var datepkrs = [],
	...
	function buildCalendar() {
		...
		// calendarContainer.style.cssText = 'display: none; position: absolute; z-index: 100;'; updated line

	    calendarContainer.style.cssText = 'display: none; position: absolute; z-index: 100;';
		
		// calendarContainer.style.cssText = 'display: none; position: absolute; top: ' + (inputTop + this.element.offsetHeight) + 'px; left: ' + inputLeft + 'px; z-index: 100;'; original line
		...
		// document.getElementById("ccontainer").appendChild(calendarContainer); updated line

		document.getElementById("ccontainer").appendChild(calendarContainer);

		// document.body.appendChild(calendarContainer); original line
	...
	return function(elementId, userConfig) {
		...
		this.open = function(e) {
			...
			foreach(datepkrs, function(datepkr) {
				if(datepkr != self) {
					datepkr.close();
				}
			});
