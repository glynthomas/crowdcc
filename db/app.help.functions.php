...
* 
* help * db * functions.php  * functions for crowdcc platform
*
*
*/
...
function signin_check($mysqli) {
  ...
  if(isset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['signin_string'])) {

         $user_id = $_SESSION['user_id'];
         $signin_string = $_SESSION['signin_string']; 
         $uname = $_SESSION['username'];
         $user_browser = $_SERVER['HTTP_USER_AGENT']; // Get the user-agent string of the user.

    switch(true) {
        
      case($stmt = $mysqli->prepare("SELECT cc_passcode FROM regist_members WHERE user_id = ? LIMIT 1")):
         ...
         $stmt->bind_result($passcode);               // get variables from result.
         $stmt->fetch();
         $stmt->close();
         $signin_check = hash('sha512', $passcode.$user_browser);

         } else {
         return false;
         }

      case($signin_check == $signin_string):
...
allowed hosts : crowdcc.dev (local dev server) , crowdcc.apc (local), crowdcc.com (internet host) (currently no subsdomains) 
     $allowed : $allowed_hosts = array('foo.example.com', 'bar.example.com');
     $allowed : $allowed = array('crowdcc.dev', 'crowdcc.apc', 'crowdcc.com');
*/
